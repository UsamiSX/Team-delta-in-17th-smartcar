...
int16 MT9V03X_CFG[CONFIG_FINISH][2]=
{
    ...
    {EXP_TIME,          150}, //曝光时间          摄像头收到后会自动计算出最大曝光时间，如果设置过大则设置为计算出来的最大曝光值
...
void SendLine()
{
    ...
    {
        if(rpts0s[i][0] >= 0 && rpts0s[i][0]<= 376 && rpts0s[i][1] >= 0 && rpts0s[i][1]<= 240)
        {
//                        lcd_drawpoint(rpts0s[i][0]/2, rpts0s[i][1]/2, GREEN);
            uart_putchar(UART_2, (uint8)(rpts0s[i][0] / 2));
            uart_putchar(UART_2, (uint8)(rpts0s[i][1] / 2));
        }
    }
    printf("RIGHT");
    for(int i = 0; i < rpts1s_num; i++)
    {
        if(rpts1s[i][0] >= 0 && rpts1s[i][0]<= 376 && rpts1s[i][1] >= 0 && rpts1s[i][1]<= 240)
        {
//                        lcd_drawpoint(rpts1s[i][0]/2, rpts1s[i][1]/2, YELLOW);
            uart_putchar(UART_2, (uint8)(rpts1s[i][0] / 2));
            uart_putchar(UART_2, (uint8)(rpts1s[i][1] / 2));
        }
    }
    printf("STOP");
}
